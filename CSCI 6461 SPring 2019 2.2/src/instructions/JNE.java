...
public class JNE extends Instructions{
	/**
	 * Jump If Not Equal:
If c(r) != 0, then PC <- EA 
Else PC <- PC + 1
	...
	public void execute(String instruction, Registers register, Memory memory) {
		...
		// TODO Auto-generated method stub
				...
				if(!instruction.substring(0, 6).equals("001011")) {
					System.out.println("Error, not a Jump not Equal instruction");
				}
				...
				//Extract r,x,i, and effective address (EA)
					...
					if(UnitConverter.binaryStringToInteger(register.getGPRj(r)) != 0)  {
						System.out.println("Register Rx != 0, set PC <- EA");
						register.setPC(effectiveAddress);
						
					}
					else {
						System.out.println("Register Rx = 0, PC + 1");
	...
	public String printMessage() {
		// TODO Auto-generated method stub
		System.out.println("JNE Instruction: r = " + r +", ix = " + ix +", i = " + i + ", address = " + address );

String message = "JNE Instruction: r = " + r +", ix = " + ix +", i = " + i + ", address = " + address ;
return "JNE Instruction: r = " + r +", ix = " + ix +", i = " + i + ", address = " + address;
